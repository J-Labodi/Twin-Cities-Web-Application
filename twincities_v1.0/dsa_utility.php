...
// function to call Birmingham pictures from flickr
...
/* The function starting from here are miscellaneous functions that support the main functions, which are transferring the database held
data into an XML file and then creating a RSS feed from the XML file. */
...
function determineArray($number) {
    switch($number) {   //switch function used to return appropriate values for the four numeric values
        ...
        case 3:
            ...
            break;
        // No default value intended because we do not desire any rss to be printed twice, and the number will be given internally
...
/* In the RSS, the entire descriptions of the pois are about a paragraph and do not go along well with other attributes that are relatively
...
/*The function below will take the number assigned to the entity and make XML file called entity.xml inside the XML_files directory. For
...
function makeXML($num) {
    ...
    $dir = '/Applications/XAMPP/xamppfiles/htdocs/twincities/XML_files';   //specifying the directory to store the xml files -- CHANGED MAC
...
function xml_to_rss($number) {
    ...
    $dir = '/Applications/XAMPP/xamppfiles/htdocs/twincities/XML_files';    //CHANGED TO MAC PATH.           //specifying the exact path to the XML file
    $dir.= $filename;
